...
- [ ] **Oracle tile release** (blocks a green CI): create `oracle-20260912` in `orkitec/velorki-data` with the recorded W20_N30/W25_N60 tiles and push the prune fix — staged in the session scratchpad as `publish-oracle-tiles.sh`; see tools/brouter-oracle/README.md. The nightly `integration.yml` serves W20_N30 from that release to the emulator and fails with an explicit message until it exists, so it is red on every run so far.
- [ ] **GitHub Releases tile mirror** ([orkitec/velorki-data](https://github.com/orkitec/velorki-data)): free hosting of the rd5 set as release assets, published monthly by `publish-tiles.yml` (smoke-tested with three tiles: `https://github.com/orkitec/velorki-data/releases/download/tiles-20260913/`). GitHub caps a release at 1,000 assets and the planet is 1,142 tiles, so a full snapshot is sharded over releases (`tiles-YYYYMMDD`, `-s2`, …) with a per-shard `manifest.json`; `latest.json` in the repo lists every shard. Until the app reads `latest.json` and merges the shard manifests, one `VELORKI_SEGMENTS_URL` covers one shard (all of Europe fits in shard 1). Decide: VPS mirror, GitHub shards, or both; then run the planet publish (1.5–2.5 h) and point `env/dev.json` at it.
