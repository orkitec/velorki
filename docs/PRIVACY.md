...
If you create a share link for a route or a ride, that route or ride — its
...
A shared item is kept for **one year** and then deleted automatically. You can
delete it earlier from within the app. Viewing a shared link requires no
account.
...
or a device identifier — and receives the purchase receipt from the store. Our
relay sends that anonymous id to RevenueCat to check whether your subscription
is active, and to nothing else.
...
| AI prompts | not stored by us beyond what the logs above contain | — |
...
not remove share links you created — delete those first — and it does not
remove anything you uploaded to Strava or RideWithGPS.
...
Orkitec — hello@orkitec.com. For security reports see `SECURITY.md`.
