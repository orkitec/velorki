...
The sheet under the map has three pages, a swipe apart, with three dots under the figures saying which is up. A new ride starts on the figures. The sheet scrolls at any height and moves by its handle or its title, or by any of it when there is nothing to scroll, as during a ride. Before a ride, pulled all the way down, it folds into the navigation bar and leaves only the handle above the tabs, so the map is free; drag the handle up to bring it back. During a ride the tabs are away, and the sheet folds into a bar of the same shape with the ride's first four figures, distance, speed, average and ascent, and the map above it; a dot on it means paused. It has no buttons: tap it or pull it up to bring the sheet back.
...
- If the recording service is still alive when you come back, the app opens on the Record tab, quietly reattaches and carries on.
- If it is not, the app opens on the Record tab and asks at once, **Unfinished ride**: "A ride from 16 Sept 2026 was never finished. 42.1 km and 2 h 10 min are saved. Continue it or finish it now?" with three answers:
...
The dialog cannot be dismissed without answering, so a recovered ride is never silently lost. It is asked once per start of the app; a file or a shared route the app was opened with waits for the answer and opens after it.
