...
- the distance, ascent, descent and duration,
- **SAVE AS**, a switch between **Route** and **Ride**.

The map stays at the top while the pages under it scroll, with dots under the map saying which page is up. One swipe to the left is the elevation profile. When the file has turns or points of interest, one more swipe is the **CUE SHEET**, every turn and point of interest with its distance from the start, folded to eight lines with **Show all**. Tap a line and the map pans there at the zoom you have, pinned with its name; tap a marker on the map and the cue sheet comes up with its line selected and scrolled into view. A selected line opens with what there is to know, a hazard's note or the plain manoeuvre under the author's words.
