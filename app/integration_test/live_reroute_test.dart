...
Future<void> _leaveAndComeBack(WidgetTester tester, RerouteMode mode) async {
  ...
  final path = await _wayRound(router.inner!, line);
  debugPrint(
    'VELORKI_REROUTE ${mode.name}: plan ${polylineLengthMeters(line).round()} '
    'm, ride ${polylineLengthMeters(path).round()} m',
  );
  await rideSimulatorAlong(tester, path, speedMps: _speedMps);
  ...
  switch (mode) {
    ...
    case RerouteMode.newRoute:
      ...
      // Riding on, it is re-planned again only for a real departure, a good
      // way from where the last one started: not in a loop.
      final asked = router.calls;
      await pumpFor(tester, const Duration(seconds: 20));
      final since = router.calls - asked;
      expect(since, lessThanOrEqualTo(1), reason: 'no re-planning loop');
    case RerouteMode.off:
      // Longer off the route than any way back or new route waits for.
      await pumpFor(tester, const Duration(seconds: 50));
...
/// [line] up to a point, a way round a stretch of it through two points to
/// one side, back onto it, and on to its end.
///
...
Future<List<LatLng>> _wayRound(RoutingBackend router, List<LatLng> line) async {
