// Port of btools.router.KinematicNoCostPath (BRouter v1.7.10).
//
// The path-instance of the kinematic model (the variant without the
// costfactor term; upstream is a copy of KinematicPath with three lines
// changed, and so is this file)
...
final class KinematicNoCostPath extends OsmPath {
  ...
  void init(OsmPath orig) {
    final origin = orig as KinematicNoCostPath;
    _ekin = origin._ekin;
  ...
  double processWaySection(
    ...
    final km = rc.pm as KinematicNoCostModel;
  ...
  double evolveDistance(KinematicNoCostModel km, double dist, double deltaH, double fAir) {
    // elevation force
  ...
  bool definitlyWorseThan(OsmPath path) {
    final p = path as KinematicNoCostPath;
